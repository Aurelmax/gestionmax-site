  ...
  useEffect(() => {
    if (slug) {
      try {
        ...
        const tagsString = post.frontmatter.tags ? post.frontmatter.tags.join(', ') : '';
        
        // Mettre à jour l'état avec les données de l'article
        setFormData({
          title: post.frontmatter.title || '',
          date: post.frontmatter.date || new Date().toISOString().split('T')[0],
          author: post.frontmatter.author || 'Aurélien Lavayssière',
...
${formData.content}`;
      ...
      alert('Article mis à jour avec succès ! Dans une implémentation réelle, le fichier Markdown serait mis à jour.');
